...
class SerialService : Service(), SerialListener {
    ...
    private inner class QueueItem internal constructor(var type: QueueType, var data: ByteArray?, var e: Exception?)
    ...
    fun disconnect() {
        ...
        cancelNotification()
        if (socket != null) {
            (socket as SerialSocket).disconnect()
            socket = null
        }
    }
    ...
    private fun createNotification() {
        ...
        val builder = NotificationCompat.Builder(this, Constants.NOTIFICATION_CHANNEL)
                ...
                .setContentText(if (socket != null) "Connected to " + (socket as SerialSocket).name else "Background Service")
